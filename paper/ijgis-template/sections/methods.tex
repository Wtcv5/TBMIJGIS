\label{sec:methods}
This section formalises rock--TBM interpretation as an
excavation-step-indexed spatial entity-relation graph. The method starts from
a TSP-derived rock voxel field, an advancing parameterized TBM surface, and
aligned monitoring records under a common excavation-step index. It then
constructs a dynamic rock--TBM spatial relation graph at each excavation step
using active-zone, distance, normal-compatibility, and excavation-state
constraints. Finally, weighted rock--machine edges are aggregated by TBM
component to compute a component $\times$ excavation-step geological anomaly
index. This field supports excavation anomaly indexing,
component-indexed querying, and response-aligned interpretation while retaining
traceable edge contributions.
...
defines geometry-constrained sparse graph construction as relation-support
construction, and Section~\ref{sec:interpretable_computation} defines
component-indexed geological anomaly index computation from screened
rock--machine edges.
...
\]
aggregates those relations into a component-indexed geological anomaly
index. The aligned monitoring records are used to read this field against
...
\caption{Method overview of the dynamic rock--TBM spatial relation graph
computation. (a) Sparse graph construction from active rock
...
excavation-step geological anomaly index computed by aggregating weighted
...
indices are formalised under the common excavation-step reference.

\begin{table}[htbp]
\centering
\caption{Entities, relations, and indices in the geometry-constrained rock--TBM interaction
graph computation.}
...
$A_c(t)$ & Sum of geometry weights for component $c$ & Auxiliary exposure metric \\
$I_c(t)$ & Geometry-weighted anomaly intensity for component $c$ & Main component-level index \\
...
$\eta_{\min}$ are set as listed in Table~\ref{tab:active_zone_parameters}. The
...
\subsection{Component-indexed geological anomaly index computation}
\label{sec:interpretable_computation}
Once the graph is constructed, the central graph computation is to aggregate
screened rock--machine edges into a component-indexed geological anomaly index.
Response alignment and edge-level query are downstream uses of this
field rather than separate method branches.

\begin{algorithm}[htbp]
\caption{Component-indexed geological anomaly index computation}
...
$\Delta I_c(t)$, it forms the component $\times$ excavation-step geological
anomaly index. $A_c(t)$ is retained as an auxiliary exposure metric, not as
part of the geological anomaly index itself. In this paper, ``field'' refers to a
discrete index over the product domain of TBM components and excavation
steps, rather than a continuous physical field in 3D space. The field denotes
the component $\times$ excavation-step matrix of $I_c(t)$ and $\Delta I_c(t)$
values, whereas an index denotes a downstream use of that matrix through
...
response alignment and can be queried back to edge contributions. For response
...
diagnostic question is whether component-indexed indices co-vary with
...
association links the graph-computed index to TBM response change; it is
interpreted as index--residual co-variation within the evaluated chainage
interval, not as causal attribution. The choice of component-level indices, comparison summaries,
and edge-level provenance analysis is part of the case evaluation
design and is specified in Section~\ref{sec:event-evaluation-design}.
