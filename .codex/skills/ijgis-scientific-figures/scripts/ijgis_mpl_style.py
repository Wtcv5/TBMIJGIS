"""Reusable Matplotlib defaults for IJGIS-style scientific figures.

Copy these helpers into project plotting code when a stable publication style is
needed.  The script is intentionally dependency-light and does not require LaTeX.

For project use, the canonical copy lives in experiments/src/visualization/style.py.
This file serves as the Skill's reference implementation.
"""
...
def setup_unwrapped_surface_axes(
    ...
    Args:
        ...
        shield_length: total shield length for aspect ratio computation.
        shield_radius: shield radius for circumference computation.
...
def save_publication_figure(fig, path: str | Path, dpi: int = 600) -> None:
...
def audit_figure_dpi(path: str | Path, min_dpi: int = 300) -> dict:
    ...
    # Estimate DPI from embedded metadata
